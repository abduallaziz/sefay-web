...
interface Item { name: string; price: string; type: string }
interface Props { items: Item[]; onChange: (items: Item[]) => void }

export default function TailorSetup({ items, onChange }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 36px', gap: '6px', marginBottom: '4px' }}>
        {[isAr ? 'الخدمة' : 'Service', isAr ? 'السعر' : 'Price', isAr ? 'النوع' : 'Type', ''].map((h, i) => (
      ...
      {items.map((item, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 36px', gap: '6px', alignItems: 'center' }}>
          <input className="form-input" value={item.name} onChange={e => update(i, 'name', e.target.value)} placeholder={isAr ? 'خياطة ثوب' : 'Thobe Tailoring'} />
          <input className="form-input" type="number" value={item.price} onChange={e => update(i, 'price', e.target.value)} placeholder="0" />
          <input className="form-input" value={item.type} onChange={e => update(i, 'type', e.target.value)} placeholder={isAr ? 'ثوب' : 'Thobe'} />
      ...
      <button onClick={() => onChange([...items, { name: '', price: '', type: '' }])}
