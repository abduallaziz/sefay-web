...
export default function RestaurantSetup({ items, onChange }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 36px', gap: '6px', marginBottom: '4px' }}>
        {[isAr ? 'اسم الوجبة' : 'Meal', isAr ? 'السعر' : 'Price', isAr ? 'الفئة' : 'Category', isAr ? 'وقت التحضير (د)' : 'Prep Time', isAr ? 'المخزون' : 'Stock', ''].map((h, i) => (
      ...
      {items.map((item, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 36px', gap: '6px', alignItems: 'center' }}>
          <input className="form-input" value={item.name} onChange={e => update(i, 'name', e.target.value)} placeholder={isAr ? 'برجر' : 'Burger'} />
          <input className="form-input" type="number" value={item.price} onChange={e => update(i, 'price', e.target.value)} placeholder="0" />
          <input className="form-input" value={item.category ?? ''} onChange={e => update(i, 'category', e.target.value)} placeholder={isAr ? 'وجبات رئيسية' : 'Main Course'} />
