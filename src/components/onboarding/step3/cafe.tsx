  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 36px', gap: '6px', marginBottom: '4px' }}>
        {[isAr ? 'اسم الصنف' : 'Item', isAr ? 'السعر' : 'Price', isAr ? 'الفئة' : 'Category', isAr ? 'المدة (د)' : 'Duration', isAr ? 'المخزون' : 'Stock', ''].map((h, i) => (
      ...
      {items.map((item, i) => (
        <div key={i} style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 36px', gap: '6px', alignItems: 'center' }}>
          ...
          <input className="form-input" value={item.category ?? ''} onChange={e => update(i, 'category', e.target.value)} placeholder={isAr ? 'مشروبات ساخنة' : 'Hot Drinks'} />
